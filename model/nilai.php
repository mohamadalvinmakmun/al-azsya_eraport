...
class Nilai
{
    ...
    public function hitungNilaiAkhir($cp1, $cp2, $cp3, $cp4, $uts, $uas) {
        // Menghitung rata-rata dengan COALESCE
        $rata = (
            (isset($cp1) ? $cp1 : 0) +
            (isset($cp2) ? $cp2 : 0) +
            (isset($cp3) ? $cp3 : 0) +
        ...
        // Menentukan nilai akhir berdasarkan rata-rata
        if ($rata > 90) {
            $nilai_akhir = "A";
        } elseif ($rata > 70) {
            $nilai_akhir = "B";
    ...
    public function ubah_nilai($id_mapel, $cp1, $cp2, $cp3, $cp4, $uts, $uas)
    {
        ...
        $smt = CURRENT_SEMESTER;
        // echo $this->nis_siswa . " ", $id_mapel . " ", $smt . " ", $cp1 . " ", $cp2 . " ", $cp3 . " ", $cp4 . " ", $uts . " ", $uas;
        // die;
        // $query = "CALL ubah_nilai('$this->nis_siswa', '$id_mapel', '$smt', '$cp1', '$cp2', '$cp3', '$cp4', '$uts', '$uas')";
        $nilaiAkhir = $this->hitungNilaiAkhir($cp1, $cp2, $cp3, $cp4, $uts, $uas);
        $query = "UPDATE nilai 
                    SET cp1 = $cp1, 
                    cp2 = $cp2, cp3 = $cp3, cp4 = $cp4, uts = $uts, uas = $uas, nilai_akhir = '$nilaiAkhir' WHERE nis = $this->nis_siswa AND id_mapel = $id_mapel AND semester = $smt";
